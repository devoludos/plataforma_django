...
const submitForm = () => {
    form.post("/administrador/usuarios/store/", {
        onSuccess: () => {
            show.value = false
            form.clearErrors()
            form.reset()
...
<template>
    <BaseDrawer v-model:show="show" title="Registrar Nuevo Usuario"
        subtitle="Completa los datos para crear una nueva cuenta." icon="pi pi-user-plus" confirmLabel="Guardar Usuario"
        :loading="form.processing"
        :disabled="!form.isDirty || !form.first_name || !form.last_name || !form.email || !form.password"
